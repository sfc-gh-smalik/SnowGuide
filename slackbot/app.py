...
class SlackChatBot:
    def __init__(self):
        # Initialize Snowflake connection (local or container)
        self.conn = get_connection()
        self.cursor = self.conn.cursor()
        
        #### Handle either Cortex or OpenAI API
        self.llm_api = os.getenv("LLM_API", CORTEX_LLM).lower()
        if (self.llm_api not in [CORTEX_LLM, OPENAI_LLM]):
            raise Exception(f"Unknown LLM API {self.llm_api}")
        logging.info(f"using LLM_API {self.llm_api}")
        ...
        if self.service_metadata:
            ...
            logging.info(f"self.selected_service={self.selected_service}")
    ...
    def complete(self, prompt):
        ...
        result = None
        logging.info(f"Answering request with {self.llm_api}")
                    ...
                    while has_more:
                        ...
                        for message in result.get('messages', []):
                            try:
                                ...
                                # Suppress can't delete message errors
                                if (str(e).find("cant_delete_message") < 0):
                ...
                try:
                    ...
                    for message in result.get('messages', []):
                        try:
                            ...
                            # Suppress can't delete error messages
                            if (str(e).find("cant_delete_message") < 0):
